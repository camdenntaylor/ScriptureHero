  ...
  return (
    <div className={`app-shell ${welcome ? 'landing-shell' : 'community-shell'}`}>
      ...
      <header className="site-header">
        ...
        {welcome ? <><nav className="landing-nav" aria-label="Main navigation"><a href="#home">The community</a><a href="#heroes">Scripture Heroes</a></nav><a className="button button-outline" href="#home">Come on in <span aria-hidden="true">↗</span></a></> : <><p className="header-motto">A little more connected. A little more you.</p><div className="demo-profile"><div><strong>Avery</strong><span>Demo account</span></div><span className="avatar avatar-gold" aria-hidden="true">AV</span></div></>}
      </header>

      {welcome ? <LandingPage /> : <div className="workspace"><aside className="workspace-sidebar"><p className="sidebar-label">YOUR LITTLE CORNER</p><nav className="side-nav" aria-label="Main navigation">{navigation.map(item => <a href={`#${item.screen}`} key={item.screen} aria-current={app.screen === item.screen ? 'page' : undefined}><Icon name={item.icon} />{item.label}</a>)}</nav><div className="sidebar-reflection"><Icon name="sun" size={26} /><p>Faith grows<br />when we share it.</p><span>Make yourself at home.</span></div><p className="sidebar-prototype">UX prototype<br />A fictional community</p></aside><main id="main-content" className="workspace-main" tabIndex={-1}>{app.screen === 'home' ? <HomePage app={app} /> : <HeroesPage app={app} />}</main></div>}
